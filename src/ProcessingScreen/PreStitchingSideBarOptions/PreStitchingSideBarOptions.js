...
const PreStitchingSideBarOptions = ({ setOverlayImageData }) => {
  ...
  const shouldRenderMultispectral =
    progress.multispectral && progress.multispectral.progress !== 0;
        ...
        {processingIsLoading && (
          <Container>
            <Typography>
              {shouldRenderRGB
                ? "RGB"
                : shouldRenderMultispectral
                ? "RGB Completo"
                : "Comenzando análisis RGB"}
            </Typography>
            <LinearProgressWithLabel
              value={shouldRenderMultispectral ? 100 : progress.rgb?.progress}
              variant={
                shouldRenderRGB || shouldRenderMultispectral
                  ? "determinate"
                  : "indeterminate"
              }
            />
        ...
        {processingIsLoading && (
          <Container sx={{ mt: 2 }}>
            <Typography>
              {shouldRenderMultispectral
                ? "MULTISPECTRAL"
                : "Comenzando análisis MULTIESPECTRAL"}
            </Typography>
            <LinearProgressWithLabel
              value={progress.multispectral?.progress}
              variant={
                shouldRenderMultispectral ? "determinate" : "indeterminate"
              }
            />
            {shouldRenderMultispectral && (
              <Typography variant="body2" color="textSecondary">{`${Math.round(
